...
class TechniqueBalancer:
    ...
    def balance(cls, draft: TechniqueDraft) -> Technique:
        ...
        name = (draft.name.strip() or "Technika Bez Nazwy")[:22]
        ...
        if archetype == "impact":
            ...
            startup += 0.035
            knockback *= 1.18
            note = "mocny finisher, krótki zasięg"
        elif archetype == "projectile":
            damage *= 0.86
            reach *= 1.55
            recovery += 0.045
            aura_cost += 2
            note = "kontrola dystansu, słabszy kontakt"
            ...
            recovery -= 0.025
            mobility *= 1.65
            note = "szybkie wejście, mniejszy burst"
        else:  # counter
            damage *= 1.02
            reach *= 0.88
            startup += 0.05
        ...
        if overload:
            balance_note = f"AI: {note}; nadmiar mocy skompensowany kosztem aury i recovery."
        else:
            balance_note = f"AI: {note}; parametry mieszczą się w budżecie turniejowym."
...
LIGHT_ATTACK = AttackSpec("Szybki", 7, 0.08, 0.08, 0.16, 78, 135, 8, 0.17)
HEAVY_ATTACK = AttackSpec("Mocny", 13, 0.18, 0.10, 0.30, 104, 260, 13, 0.28)
